...
import { query, create, update, remove, showSiteName } from '../services/publish'
...
export default modelExtend(pageModel, {
  ...
  effects: {
    ...
    *create ({ payload }, { call, put }) {
      ...
      const data = yield call(create, { state: 1, ...newpublish })
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ publish }) => publish.currentItem.id)
      const newpublish = {
        note: payload.note,
        id,
      }
      const data = yield call(update, newpublish)
      if (data.code === 200) {
        yield put({ type: 'hideModal' })
